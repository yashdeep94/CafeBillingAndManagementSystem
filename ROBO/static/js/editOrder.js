...
async function getOrdersById(orderId) {
    const response = await fetch(`/getOrdersById/${orderId}/`);
    let data = await response.json();
    return data;
};
let itemData;
...
getItems().then(data => {
    getMainCategory().then(catData => {
        ...
        getPaymentModes().then(data => {
            ...
            let orderId = window.location.href.slice(38, -1);
            orderForm.setAttribute("action", window.location.pathname);
            getOrdersById(orderId).then(orderData => {
                let parsedData = JSON.parse(orderData);
                document.getElementById("tableNo").value = parsedData.name;
                paymentType.value = parsedData.payment_type_id;
                let quantityItems = JSON.parse(parsedData.quantity_items);
                Object.keys(quantityItems).forEach(key => {
                    selectedItemsList.push(parseInt(key));
                    quantitySelected.push(parseInt(quantityItems[key]));
                    let index = itemData.item_id.indexOf(parseInt(key));
                    for (let i = 0; i < parseInt(quantityItems[key]); i++){
                        ...
                        svgDiv.classList.add(parseInt(parseInt(key)));
                        ...
                        line1.classList.add(parseInt(parseInt(key)));
                        ...
                        line2.classList.add(parseInt(parseInt(key)));
                ...
                itemsList.value = selectedItemsList;
                quantity.value = quantitySelected;
                total_block.innerHTML = `Total : ${parsedData.total}`;
        ...
        itemData.name.forEach((element, count) => {
            if (itemData.item_size[count] != "None") {
                ...
                if (`${itemData.maincategory[count]}` in categoryWithIdsThreeCols) {
                    categoryWithIdsThreeCols[`${itemData.maincategory[count]}`]["html"] += `<div>${element}</div>
                                                                                                <div>
        ...
        Object.keys(categoryWithIds).forEach(element => {
            if (categoryWithIds[element]["mainCategory"] in finalCategoriesHtml) {
                finalCategoriesHtml[categoryWithIds[element]["mainCategory"]] += `<div>${categoryWithIds[element]["name"]}</div>
                                                        <div>
                ...
                finalCategoriesHtml[categoryWithIds[element]["mainCategory"]] += `<div class="fiveCols">
                                                        ...
                                                        <div class="headingBold quantityNone">Quantity</div>
                                                        <div>${categoryWithIds[element]["name"]}</div>
                                                        <div>
                                                            <input type="checkbox" id="${categoryWithIds[element]["Small"]}">
                                                            <input type="number" id="${categoryWithIds[element]["Small"]}_quantity_inline" class="quantityInput displaySmall" value="1">
                                                        </div>
